...
```go
import bindings "github.com/git-hyagi/pulp-bindings-go/bindings"
```
...
For using other server than the one defined on index 0 set context value `bindings.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), bindings.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `bindings.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), bindings.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `bindings.ContextOperationServerIndices` and `bindings.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), bindings.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), bindings.ContextOperationServerVariables, map[string]map[string]string{
...
```go
auth := context.WithValue(context.Background(), bindings.ContextBasicAuth, bindings.BasicAuth{
	UserName: "username",
	Password: "password",
})
...
auth := context.WithValue(
		context.Background(),
		bindings.ContextAPIKeys,
		map[string]bindings.APIKey{
